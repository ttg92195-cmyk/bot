...
let lastDBError = ''; // ချိတ်ဆက်မှု အမှားအကြောင်းရင်း သိမ်းမယ်
...
bot.action('admin_dbstatus', async (ctx) => {
  ...
  try {
    ...
    if (!dbConnected && lastDBError) {
      statusText += `\n❌ <b>Error:</b> <code>${escapeHtml(lastDBError)}</code>\n`;
    }

    if (!dbConnected) {
      statusText += `\n⚠️ <b>MongoDB ချိတ်ဆက်မရပါ!</b>\n\nဖြေရှင်းနည်း:\n`;
      statusText += `1. MongoDB Atlas → Cluster Pause ဖြစ်နေလား စစ်ပါ\n`;
      statusText += `2. Network Access → 0.0.0.0/0 ရှိမရှိ စစ်ပါ\n`;
      statusText += `3. Database Access → User/Password စစ်ပါ\n`;
      statusText += `4. Railway → <code>MONGODB_URI</code> မှန်မှန် စစ်ပါ`;
    }
...
// /dbstatus command ကို အောက်မှာ သတ်မှတ်ထားပြီးပါပြီ (Duplicate removed)
...
bot.action('admin_reconnect_db', async (ctx) => {
  ...
  try {
    await ctx.editMessageText('🔄 <b>MongoDB ပြန်ချိတ်ဆက်နေပါသည်...</b>', { parse_mode: 'HTML' });
    ...
    if (dbConnected && readyState === 1) {
      ...
      await ctx.editMessageText(
        `✅ <b>MongoDB ပြန်ချိတ်ဆက်ပြီးပါပြီ!</b>\n\n🔌 State: ${stateNames[readyState]}\n🎬 Movies: ${movieCount}`,
        {
          parse_mode: 'HTML',
          ...Markup.inlineKeyboard([
            [Markup.button.callback('🔙 Admin Panel', 'admin_back')]
    ...
    console.error('admin_reconnect_db error:', err.message);
    try {
      ctx.reply('❌ MongoDB ပြန်ချိတ်ဆက်မှု မအောင်မြင်ပါ။ /dbstatus နဲ့ စစ်ပါ။');
    } catch (e) {}
  }
});
